...
# JOEY EDIT:
def extract_and_remove_prefix(prefix, state_dict):
    new_state_dict = {}
    for key in state_dict.keys():
        if key.startswith(prefix):
            prefix_removed = key.replace(prefix, "", 1)
            if prefix_removed.startswith("."):
                prefix_removed = prefix_removed[1:]

            new_state_dict[prefix_removed] = state_dict[key]
...
def convert_megatron_checkpoint(args, input_state_dict, config):
    ...
    # JOEY EDIT:
    model = input_state_dict["state_dict"]
    # JOEY EDIT:
    model = extract_and_remove_prefix("model", model)

    # The language model.
    # lm = model["language_model"]
    # JOEY EDIT:
    lm = extract_and_remove_prefix("language_model", model)

    # lm = model.language_model
    # The embeddings.
    # embeddings = lm["embedding"]
    # JOEY EDIT:
    embeddings = extract_and_remove_prefix("embedding", lm)
    ...
    # JOEY EDIT:
    transformer = extract_and_remove_prefix("encoder", lm)
...
# JOEY EDIT:
def load_config(config, state_dict):
    # print(state_dict.keys())
    # print(state_dict["hyper_parameters"]["cfg"])

    cfg_checkpoint = state_dict["hyper_parameters"]["cfg"]
    # print(sorted(cfg_checkpoint.keys()))
    config.vocab_size = 64000
    # config.n_positions = cfg_checkpoint["max_position_embeddings"]
    config.n_positions = 2048
    config.n_embd = cfg_checkpoint["hidden_size"]
    config.n_layer = cfg_checkpoint["num_layers"]
    config.n_head = cfg_checkpoint["num_attention_heads"]
    config.n_inner = cfg_checkpoint["ffn_hidden_size"]
...
def main():
    ...
    # JOEY EDIT:
    load_config(config, input_state_dict)
